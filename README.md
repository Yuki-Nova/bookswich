...
> ⚠ **2026-08-06 重大调整**：本项目已去 RAG 化。原知识库问答（检索/向量/DeepSeek）功能已全部移除，知识库路线改为 **Hermes + Obsidian**（Obsidian 双向链接建库，Hermes 负责笔记操作）。本项目只保留「解析 + 格式修正 + 下载」。

## 功能特性

- 📚 **上传即用**：任意 PDF 教材，上传后自动分批解析（MinerU OCR，公式转 LaTeX、表格转 HTML）
- 🧠 **结构修正**：不信任 OCR 的标题推断，按教材编号体系（章→节→小节）重新打标，章节结构准确
- 🖼️ **图片完整保留**：解析图片随批次落盘，导出打包为 zip（md + images/ 子目录），Obsidian/Typora 打开即显图
- 🗂️ **Obsidian 一键导入**：按章拆分（每章 30~370KB 秒开，性能友好）+ MOC 总览页（[[双链]] 导航）+ 各章独立 images/；图片转 OSS 外链（vault 纯文本，多端同步轻量）；WebDAV 同步方案下导入即同步（无需下载）
- 📥 **Markdown 下载**：整本 / 单章 / 原始 OCR / Obsidian 版（zip 包），Typora/Obsidian 友好（表格原样保留、公式规范化、无超长行）
- 📊 **配额透明**：MinerU 免费额度（优先 2 解析每日 1000 页；每日总限 5000 份文件，一份 PDF 无论页数均按 1 计）实时显示，解析进度可视化
- 🎨 **Vercel 风格 UI**：2026-08-07 前端重设计（sticky 导航 + 拖拽上传 + 教材卡片列表 + 状态徽标，移动端适配）

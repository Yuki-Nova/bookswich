...
- [x] **L-3 新组件 SidebarPanel.vue**（props: books/quota/settings/parseTask）：
      ...
      - **已入库教材卡片**（2026-08-11 用户拍板从右侧移入，仿 MinerU「最近文件」）：
        紧凑卡片（标题截断 + 状态徽标 + 页数/进度 + 章节下拉 + ⬇ZIP/原/📓版/📥导入/🗑）
...
2. 教材列表归属：**左侧边栏**（2026-08-11 用户拍板，仿 MinerU「最近文件」——限额/任务/教材三卡
   集中左侧，右侧主区只留上传 + 对比预览占位；原右侧列表死样式已清）

...
## 结构重建修复：阿拉伯数字章/节标题识别（2026-08-09 已批准执行完毕）

**背景（已实锤，含服务器实测）**：
- b8《基础医学概论》structure.json = 345 章（真实应为 ~11 章），b7《管理学》= 16 章正常
- 根因链：RE_CH_LEVEL1/RE_LEVEL2_JIE 只认中文数字，第1章/第1节（阿拉伯数字）漏判
  → _detect_chapter_style 误判无编号 → hash 兜底；toc_titles 提取同样漏阿拉伯数字
  → 白名单空 → 所有 # 行（封面/案例2-1/问题/正文小标题）全当章 → 345 章切片
- 本地已复现；服务器代码已是最新版；b8 批次 md 完整落盘（20 批），重跑重建零配额

### P0 已做 — structure.py 正则修复
- CN_OR_AR 支持阿拉伯数字 + 第 3 章带空格变体（RE_CH_LEVEL1/JIE/LEVEL3/TOC_LINE/_extract_toc_titles/check_section_continuity 统一）
- cn_to_int 支持阿拉伯数字
- 效果：b8 345 章 → 11 章，案例/问题回归正文；b7 管理学 16 章不受影响

### P1 已做 — hash 兜底防退化
- 去掉 not toc_titles 放行：toc 白名单为空时 hash 兜底不启用（宁 0 章不切片）

### 验证已做
- pytest 22 全绿（新增 8 个阿拉伯数字回归用例）
- 服务器预演 + 正式重建：11 章；parse_status=structure_ok
- 服务器 obsidian 导出：删旧 345 目录，OSS 外链模式重导（551KB 纯文本，无 images/）
- git commit fbe746f

---

## 表格公式渲染重构（2026-08-09 已批准执行中）

**背景**：
- MinerU 表格内公式用 <eq>LaTeX</eq> 标签（非 $...$），Typora/Obsidian 不渲染
- HTML <table> 内的 $...$ 也不被 Markdown 渲染器解析
- 测试验证：HTML table → Markdown table + <eq> → $ 后公式完美渲染
- 需处理 HTML 实体（&lt; → <），否则 LaTeX misplace &

### P0 已做 — format_table_md 核心转换函数（exporter.py）
- <eq>...</eq> → $...$（含 html.unescape 实体解码）
- <tr>/<td> → | cell | cell |，colspan=N 复制内容到 N 列
- 插入 | --- | 分隔行

### P1 已做 — 集成到 _node_to_md
- <table 开头的行改调 format_table_md（旧 format_html_table 保留作回退）

### 遗留问题 ⚠（2026-08-09 用户反馈）
- colspan 复制平铺导致列错乱：带合并单元格的表格格式混乱
- 方案调整：检测到 colspan/rowspan 时降级保留 HTML 原样（方案 C），只转换无合并单元格的简单表格
- 待验证：pytest + 真实数据 + Typora 抽查

### 验证
1. 《医药应用概率统计》132 个表格导出抽查
2. pytest 27 个不回归
3. format_table_md 单元测试（test_table_md.py）

### 最终决策（2026-08-09 用户拍板）：表格保持 MinerU HTML 原样
- 尝试过：HTML→Markdown 表格转换（colspan 复制→列错乱；展开补空→复杂公式渲染翻车；
  HTML 表格内 \$...\$ Typora/Obsidian 都不渲染）
- 结论：格式正确 > 表格内公式渲染。_node_to_md 回退 format_html_table（HTML 原样），
  format_table_md 保留但不再集成（留作纯文本版导出备选）
